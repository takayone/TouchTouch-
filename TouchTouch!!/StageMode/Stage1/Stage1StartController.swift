...
class Stage1StartController: UIViewController{
    var timer = Timer()
    ...
    @objc func updateTimer(){
        ...
        if countDownTime < 1{
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) {
                let stage1GameField = Stage1GameField()
                self.present(stage1GameField, animated: true, completion: nil)
            }
