...
class Stage1GameField: UIViewController{
    ...
    @objc func handleProgressTimer(){
        ...
        if remainedTime < 1{
            ...
            if countNumbers > 9{
               let stage2StartController = Stage2StartController()
               present(stage2StartController, animated: true, completion: nil)
            }else{
               let stageResultController = StageResultController()
               stageResultController.stageResult = "Stage(Not Clear)"
